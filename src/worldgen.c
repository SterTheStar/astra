...
uint8_t getChunkBiome (short x, short z) {
  ...
  // Flat world mode: only plains biome
  if (worldgen_mode == 1) return W_plains;
...
ChunkFeature getFeatureFromAnchor (ChunkAnchor anchor) {
  ...
  // Flat world mode: no features
  if (worldgen_mode == 1) skip_feature = true;
