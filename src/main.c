  ...
  switch (packet_id) {
    ...
    case 0x07:
      if (state == STATE_CONFIGURATION) {
        ...
        printf("  Finishing astra-settingsuration\n\n");
...
int main () {
  ...
  // Load astra-settingsuration from astra-settings.ini
  FILE *fp = fopen("astra-settings.ini", "r");
  if (fp) {
    char line[256];
    while (fgets(line, sizeof(line), fp)) {
      if (sscanf(line, "port=%d", &port) == 1) continue;
      if (sscanf(line, "view_distance=%d", &view_distance) == 1) continue;
      if (sscanf(line, "worldgen_mode=%d", &worldgen_mode) == 1) continue;
      if (strstr(line, "motd=") == line) {
        char *value = line + 5; // skip "motd="
    ...
    // Create astra-settings.ini with default values
    fp = fopen("astra-settings.ini", "w");
    if (fp) {
      fprintf(fp, "port=%d\n", port);
      fprintf(fp, "view_distance=%d\n", view_distance);
      fprintf(fp, "worldgen_mode=%d\n", worldgen_mode);
      fprintf(fp, "motd=%s\n", motd);
      fclose(fp);
    }
  }
...
void astra_main (void *pvParameters) {
  main();
  vTaskDelete(NULL);
}
...
static void wifi_event_handler (void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    xTaskCreate(astra_main, "astra", 4096, NULL, 5, NULL);
  }
}
...
void wifi_init () {
  ...
  wifi_init_astra-settings_t cfg = WIFI_INIT_CONFIG_DEFAULT();
  esp_wifi_init(&cfg);

  esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, NULL, NULL);
  esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_event_handler, NULL, NULL);

  wifi_astra-settings_t wifi_astra-settings = {
    .sta = {
      .ssid = WIFI_SSID,
      .password = WIFI_PASS,
  ...
  esp_wifi_set_astra-settings(WIFI_IF_STA, &wifi_astra-settings);
